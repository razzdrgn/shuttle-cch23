...
impl Results {
	fn new(deer: &[Reindeer]) -> Results {
		...
		let magician_deer = deer
			...
			.unwrap();
		let consumer_deer = deer
			.iter()
			.reduce(|x, y| {
				if x.candies_eaten_yesterday > y.candies_eaten_yesterday {
					x
				} else {
					y
				}
		...
		let magician = format!(
			...
			magician_deer.name,
			magician_deer.snow_magic_power.unwrap()
		);
		let consumer = format!(
			"{} ate lots of candies, but also some {}",
			consumer_deer.name,
			consumer_deer.favorite_food.clone().unwrap()
		);
		Results {
			fastest,
			tallest,
...
async fn day_four_strength(Json(reindeer): Json<serde_json::Value>) -> impl IntoResponse {
	...
	let deer: Vec<Reindeer> =
		serde_json::from_value(reindeer).expect("Broke when running from_str");
	(
		StatusCode::OK,
		deer.iter()
			.map(|x| x.strength)
